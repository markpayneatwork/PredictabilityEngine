...
these.cfg <-
  ...
  mutate(pcfg=map(name,~readRDS(here(this.dir,sprintf("%s_configuration.rds",.x)))),
         db=map(name,~dbConnect(SQLite(),here(this.dir,sprintf("%s_results.sqlite",.x))))) 
...
these.cfg <- 
  ...
  these.cfg %>%
  mutate(cor.thresh=map_dbl(pcfg,~ cor.fn(length(.x@comp.years))))
...
g <- 
  ...
  geom_hline(data=skill.cfgs,
             mapping=aes(linetype="r > 0 (p=5%)",
                         yintercept=cor.thresh))+
