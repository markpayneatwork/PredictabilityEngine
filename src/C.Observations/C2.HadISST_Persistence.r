#/*##########################################################################*/
#' Create a persistence forecast based on HadISST data
#' ==========================================================================
#'
#' by Mark R Payne  
...
#'
#' Creates a persistence forecast based on HadISST data
#
...
#Working directories
base.dir <- define_dir(define_dir(file.path(pcfg@scratch.dir,"Persistence")),
                       "HadISST")
...
#There is no-need to do monthly averaging here, as we actually want
#the individual monthly anomalies
...
condexec(3,frag.cmd <- cdo("splityearmon",anom.fname,frag.prefix))
...
#  by simply tweaking the metadata properly
#/*======================================================================*/
...
for(f in frag.fnames) {
  r <- raster(f)
  date.l[[f]] <- tibble(date=getZ(r))
}
anom.meta <- bind_rows(date.l) %>%
  add_column(name=pcfg@persistence@name,.before=1) %>%
  add_column(type=pcfg@persistence@type,.after=1) %>%
  mutate(n.realizations=1,
         start.date=NA,
         fname=frag.fnames) 
