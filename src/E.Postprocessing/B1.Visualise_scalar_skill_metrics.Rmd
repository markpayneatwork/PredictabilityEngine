  ...
  for(this.stat in stat.names) {
    ...
    for(this.result in result.names) {
      ...
      stat.plt.dat <- 
        ...
        separate(calibrationMethod,c("calibration","matching","period"),
      ...
      for(this.met in metric.names) {
        ...
        met.persis.dat <-
          all.met.dat %>%
          filter(srcType=="Persistence") %>%
          dplyr::select(-calibration)  #So that it is added to all facets
        ...
        g <- ggplot(mapping=aes(x=lead/12,y=value,
                                group=type.name,colour=type.name,size=highlight))+
