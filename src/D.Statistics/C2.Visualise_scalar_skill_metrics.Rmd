...
for(this.sp in sp.names) {
  ...
  for(this.stat in stat.names) {
    ...
    for(this.result in result.names) {
      ...
      #Combine data
      stat.plt.dat <- 
        bind_rows(stat.obs,stat.pred) %>%
        mutate(date=as.Date(date),
               timeframe=ifelse(year(date)<min(year(stat.pred$date)),
                                "Historical","Near-term predictions"))

      #Plot timeseries
      g <- 
        ggplot(stat.plt.dat,
               aes(x=date,y=value,colour=srcName))+
        geom_line()+
        labs(x="Year",y=this.stat)+
        facet_grid(calibrationMethod~timeframe,scales="free_x")
      print(g) 
      ...
      for(this.met in metric.names) {
        ...
        met.persis.dat <-
          all.met.dat %>%
          filter(srcType=="Persistence") %>%
          dplyr::select(-calibrationMethod)  #So that it is added to all facets
        ...
        #Plot
        g <- ggplot(mapping=aes(x=lead/12,y=value,group=type.name,colour=type.name,size=highlight))+
