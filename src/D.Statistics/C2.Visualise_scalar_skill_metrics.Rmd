...
for(this.sp in sp.names) {
  ...
  for(this.stat in stat.names) {
    ...
    g <- 
      ggplot(stat.plt.dat,aes(x=date,y=value,colour=srcName,group=type.name))+
      geom_line()+
    ...
    for(this.met in metric.names) {
      ...
      met.dat <- 
        ...
        collect() %>%
        mutate(type.name=paste(srcType,srcName,sep="."),
               is.ensmean=realization=="ensmean") 
      ...
      #Plot
      g <- ggplot(met.dat,aes(x=leadIdx,y=value,group=type.name))+
        geom_line(aes(colour=type.name,size=is.ensmean))+
        facet_wrap(~metric,scales = "free_y")+
        coord_cartesian(ylim=y.rng)+
        scale_size_manual(values=c(0.25,1))+
        guides(size="none")+
        labs(x="Lead (number of forecasts)",y=this.met,colour="Forecast System")
      print(g)
