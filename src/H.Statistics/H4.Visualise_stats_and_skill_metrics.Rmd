---
title: Statistics and Forecast Skill Visualisations
output:
  html_document:
    keep_md: false
    toc: true
...
stat.dat <- readRDS(file.path(base.dir,PE.cfg$files$stats))
...
for(this.sp in names(sp.skill.l)) {
  ...
  for(this.ss in names(sp.ss.dat.l)) {
    ...
    g <- ggplot(stat.plt.dat,aes(x=date,y=value,colour=name))+
         geom_line()+
