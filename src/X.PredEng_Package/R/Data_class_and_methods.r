...
#' take place over all available layers
...
#' @slot ensmem.fn A function to extract the realisation, for cases where a simple index doesn't work
...
data.source <- setClass("data.source",contains="PredEng.source",
                        ...
                        prototype=list(levels=1,
                                       ...
                                       ensmem.fn=function(x) {stop("Ensmem function not specified")},
...
#' @param f  Filename on which to test the functions
#'
#' @return Displays the outputs generated by the functions
#' @export
#'
test.data.source <- function(obj,f){
  log_msg("Date function\n")
  print(obj@date_fn(f))
  log_msg("Ensmem_fn\n")
  print(obj@ensmem_fn(f))
  log_msg("Init_fn\n")
  print(obj@init_fn(f))
}
