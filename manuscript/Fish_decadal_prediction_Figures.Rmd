...
cfgs$"Blue-whiting-decadal" <-
  tibble(tbl=PE.cfg$db$metrics,
         ...
         #resultName="westwardExtentLarvae",
         calibrationMethod="MeanAdj",
         lbl="c) Blue whiting",
         var="Sal",
         fig="skill")
...
skill.dat <- 
  ...
  mutate(lead.yrs=lead/12,
         highlight=srcType=="Decadal" & srcName=="ensmean" | srcType=="Persistence" ,
...
#            colour.pal=c(base.pal[1],"grey",base.pal[3:5])),
       list(metric="MSSS",
            ylab="Mean squared error skill score (MSESS)",
            signif.y =1.1,
            ylim=c(-1,1.2),
            colour.pal=base.pal))
...
for(this.cfg in plt.cfgs) {
  ...
  met.plots[[this.cfg$metric]] <- 
    #Setup baseline
    ggplot()+
    facet_wrap(~lbl,ncol=1)+
    annotate("rect",xmin=0,xmax=10,ymin=1,ymax=this.cfg$ylim[2],fill="white",colour="black")+
...
smooth.dat <-
  ...
  map_dfr(function(this) {
    this$dat <- 
      this$mets.db[[1]] %>%
      tbl(PE.cfg$db$metrics) %>%
      filter(srcName=="ensmean" | srcType=="Persistence",
             spName==!!this$spName,
             statName==!!this$statName) %>%
      collect() %>%
      filter(calibrationMethod==this$calibrationMethod | is.na(calibrationMethod)) %>%
...
#  filter(metric %in% c("MSSS","pearson.correlation")) %>%
  ...
  mutate(smoothing=case_when(is.na(smoothing) ~ "1 year",
         ...
         dispName=ifelse(srcType=="Decadal","Ens. mean",srcType),
...
# facet.lbls <-
#   expand_grid(ylbl=levels(smooth.plt$ylbl),
#               lbl=levels(smooth.plt$lbl)) %>%
#   mutate(this.lbl=sprintf("%s)",letters[1:nrow(.)]),
#          x=0.1,y=0.99,
#          ylbl=factor(ylbl,levels(smooth.plt$ylbl)),
#          lbl=factor(lbl,levels(smooth.plt$lbl)))

g <- 
  #Setup baseline
  ggplot()+
  facet_grid(ylbl~lbl,switch = "y")+
...
tbl.dat <-
  ...
  mutate(p.value=map2_dbl(`draws_Ens. mean`,draws_Persistence,~ mean(.x <.y)),
         "Ens.mean"=map_dbl(`value_Ens. mean`,~.x),
...
# Setup configuration -------------------------------------------------------------
forecast.cfg.l <- list()
forecast.cfg.l$"Mackerel-summer" <-
  tibble(yr.rng=list(1980:2020),
         starts=list(c(1980,1990,2000,2012)),
         scalar=1/1e3,
         lbl2="Mackerel habitat (kkm²)")

# forecast.cfg.l$Bluefin <- 
#   tibble(yr.rng=list(1980:2010),
#          starts=list(c(1981,1991,2001)),
#          scalar=1/1e6,
#          lbl2="Bluefin tuna habitat (Mkm²)")


# forecast.cfg.l$"Blue-whiting-decadal" <-
#   tibble(yr.rng=list(1970:2015),
#          starts=list(c(2004,2010,2012)),
#          scalar=1/1e3,
#          lbl2="Blue whiting spawning habitat (kkm²)")


forecast.cfg <-
  bind_rows(forecast.cfg.l,.id="name") %>%
  left_join(y=these.cfg,by="name") %>%
  rowwise() %>%
  group_split()

#Loop over individual configurations
forecast.plt <- list()
fixed.lead.plt <- list()

for(this.cfg in forecast.cfg) {
  # this.cfg <- forecast.cfg[[1]]
  ...
  stat.dat <- 
    ...
    filter(spName==!!this.cfg$spName,
           ...
           calibrationMethod==!!this.cfg$calibrationMethod | is.na(calibrationMethod))%>%
    select(-field) %>%
    collect() %>%
  ...
  # Forecast plot -------------------------------------------------------------------
  forecast.plt[[this.cfg$name]] <- 
    ggplot()+
    #Forecasts
    geom_ribbon(data=forecast.real.rng,
                mapping=aes(x=year,ymax=upper,ymin=lower,fill=startYear),
                alpha=0.25)+
    geom_line(data=forecast.realmean,
  ...
  #   this.model %>%
  #   filter(realization=="realmean",
  #          leadIdx==this.lead.idx) %>%
  #   filter(year %in% this.cfg$yr.rng[[1]])
  # 
  # fixed.preds <-
  #   this.model %>%
  #   filter(realization!="realmean",
  ...
  #   pivot_wider(names_from=ptile,values_from = value)
  # 
  # fixed.lead.plt[[this.cfg$name]] <-
  #   ggplot()+
  #   #Ribbon
  #   geom_ribbon(data=fixed.preds,
...
wrap_plots(forecast.plt,ncol=1)+
  plot_annotation(tag_levels = "a",tag_suffix = ")")

# wrap_plots(fixed.lead.plt,ncol=1)+
#   plot_annotation(tag_levels = "a",tag_suffix = ")")
...
this.model <- 
  ...
  filter(srcType=="Decadal",
         srcName=="CESM.DPLE",
         year(startDate) %in% c(2004,2010,2016),
         lead<74)  %>%
...
forecast.rng <-
  this.model %>%
  filter(realization!="realmean" | realization!="ensmean") %>%
  group_by(startYear,year) %>%
  summarise(ptile=c("lower","upper"),
            value=quantile(value,probs=c(0.05,0.95)),
            .groups="drop") %>%
...
BW.plot <- 
  ggplot()+
  #Background fill
  annotate("rect",xmin=-Inf,xmax=Inf,ymax=100,ymin=-Inf,fill="white",colour="black")+
  #Forecasts
  ...
  scale_linetype_manual(values=c("dotted","longdash","dotdash"),
                        guide=guide_legend(order=1))+
  ...
  scale_shape_manual(values=c(17,NA,22),
                     guide=guide_legend(title="Observations",order=1))+
  #SSB timeseries
  geom_point(data=SSB.dat,
             mapping=aes(x=Year,y=90,size=SSB/1e6),shape=16)+
  ...
  scale_y_continuous(limits=c(80,350),expand=c(0,0))+
  theme(legend.position = "bottom",
        legend.direction = "vertical")


wrap_plots(c(forecast.plt,list(BW.plot)),nrow=1)+
  plot_annotation(tag_levels = "a",tag_suffix = ")")
...
#   mutate(signif=ifelse(value>0,NA,"ns. (p>0.05)"))
# 
# #Extract polygons
# sp.polys <-
#   skill.cfgs %>%
...
#     facet_wrap(~var,
#                ncol=2,
#                labeller = labeller(var=  c(SST="a) SST (August)",
...
#                  show.limits=TRUE)+
#     #Significance
#     geom_point(aes(x,y,shape=signif),
#                data=ns.dat,
#                size=1,col="grey")+
