...
skill.dat <- 
  ...
  mutate(lead.yrs=lead/12,
         ...
         dispName=fct_recode(dispName,
                             "Grand Ens."="grandens",
                             "CESM DPLE"="CESM.DPLE",
                             "MPI-ESM1.2-HR"="MPI-ESM1-2-HER"))
...
set1.pal <- c("#e41a1c","#377eb8","#4daf4a","#984ea3","#ff7f00","#a65628") #Set1
npg.pal <- pal_npg()(7)
base.pal <- c(npg.pal[1:2],set1.pal[-c(1:2)],"grey50")
plt.cfgs <-
  list(list(metric="pearson.correlation",
       ...
       list(metric="MSSS",
            ...
            show.signif=TRUE,
            two.panel=TRUE,
            panel.letters=letters[1:3],#c("a","c","e"),
            colour.pal=base.pal),
       list(metric="crpss",
...
for(this.cfg in plt.cfgs) {
  ...
  highlight.dat <-
    cleaned.dat %>%
    filter(highlight) %>%
    mutate(dispName=factor(dispName,c("Grand Ens.","Persistence")))
  
  #Store src data
  src.dat[[this.cfg$metric]]$dat  <- 
    cleaned.dat %>% 
    select(metric,
           stock=lbl,
           model=dispName,
           lead=lead.yrs,
           mean=value,lowerCI, upperCI)  %>% 
    pivot_longer(c(-metric,-stock,-model,-lead),names_to="statistic")
...
# Make plot -------------------------------------------------------------------------
  this.plt <- 
    ...
    if(this.cfg$show.signif) {
      ...
      signif.dat <- 
        highlight.dat %>% 
        select(lbl,lead.yrs,metric,draws,dispName) %>%
      ...
      #Store significance source data
      src.dat[[this.cfg$metric]]$signif  <-  
        signif.dat %>% 
        select(metric,
               stock=lbl,
               lead=lead.yrs,
               value=p.value) %>% 
        mutate(statistic="p.value")
...
#Write source data into a file
src.dat %>% 
  unlist(FALSE) %>% 
  bind_rows() %>% 
  arrange(metric, stock,model,lead,statistic) %>% 
  write_csv2(file=here(this.dir,"source_data","Fig2_data.csv"))
...
ensmean.metrics <- 
  ...
  #Remove blue whiting forecasts at longest timescales - something odd here
  group_by(smoothing) %>% 
  filter(!(str_detect(lbl,"Blue whiting") & lead==max(lead))) %>% 
  ungroup()
...
tbl.dat %>% 
  select(-p.value) %>%
  rename(species="lbl") %>% 
  write_csv2(file=here("manuscript","source_data","Fig3a_smoothing.csv"))
```

# Fixed lead plot

```{r}
this.lead.idx <- 6  #Corresponding to five years
...
forecast.lbl <- sprintf("Grand Ens.\n(%i year lead)",this.lead.idx-1)
fixed.lead.plt <-
  ggplot()+
  facet_wrap(~lbl,
             ncol=1,
...
src.dat.l <- list(grandens=  fl.grandens %>% 
                    select(lbl,year,value) %>% 
                    mutate(source="Grand Ens. mean"))
src.dat.l$preds <- 
  fl.preds %>% 
  mutate(year=year(date)) %>% 
  select(-date) %>% 
  rename("Grand Ens. lowerCI"=lower,
         "Grand Ens. upperCI"=upper) %>% 
  pivot_longer(c(-lbl,-year),names_to="source")
...
src.dat.l$obs <-
  ...
  mutate(source=fct_recode(source,
                           "Obs. - Habitat"="Habitat",
                           "Obs. - Roll. mean"="Rolling mean"))

src.dat.l %>% 
  bind_rows() %>% 
  rename(species="lbl") %>% 
  write_csv2(file=here("manuscript","source_data","Figure3b.csv"))
...
this.cfg <-
  ...
  mutate(yr.rng=list(1990:2020),
         starts=list(c(1990,2000,2010)),
         scalar=1/1e3,
         lbl2="Mackerel habitat (10³ km²)")
...
src.l$forecasts <-
  forecast.real.rng %>% 
  rename(source="startYear",
         lowerCI="lower",upperCI ="upper") %>% 
  mutate(source=paste(source, "forecast")) %>% 
  pivot_longer(c(-source,-year),names_to="statistic")

src.l$mean <- 
  forecast.realmean %>% 
  select(year,startYear,value) %>% 
  mutate("statistic"="mean",
         source=paste(startYear,"forecast")) %>% 
  select(-startYear)
...
src.l %>% 
  bind_rows() %>% 
  arrange(source,year,`statistic`) %>% 
  write_csv2(file=here("manuscript","source_data","Fig4_Mackerel.csv"))
...
this.cfg <- 
  ...
  mutate(yr.rng=list(2004:2019),
         ...
         scalar=1/1e3,
         lbl2="Blue whiting spawning area (10³ km²)")
...
src.l$forecast <- 
  forecast.rng %>% 
  mutate(source=paste(startYear,"forecast")) %>% 
  rename(lowerCI="lower",upperCI="upper") %>% 
  select(-startYear) %>% 
  pivot_longer(c(-source,-year),names_to="statistic")
src.l$mean <- 
  forecast.mean %>% 
...
src.l$surv <- 
  surv.dat %>% 
  select(source=lbl,year,value)
src.l$SSB <- 
  SSB.dat %>% 
  mutate(source="Biomass") %>% 
  rename(year="Year",value="SSB")
...
src.l %>% 
  ...
  write_csv2(file=here("manuscript","source_data","Fig4b_blue_Whiting.csv"))
...
```{r}
#Split out and remerge uninit metrics
uninit.dat <-
  skill.dat %>%
  filter(srcType=="CMIP6",
         srcName=="grandens")
...
uninit.comp <- 
  ...
  filter(srcType=="Decadal",
         srcName=="grandens") %>% 
  left_join(y=uninit.dat,
            by=c("lbl","calibrationMethod","spName","statName","resultName","metric"),
            suffix=c(".decadal",".uninit")) %>% 
  #Do comparison
  mutate(p.value=map2_dbl(draws.decadal,draws.uninit,~mean(.x<.y))) %>% 
  #Tidy for plotting
  mutate(lead=lead.decadal/12,
         species=fct_relabel(lbl,~gsub("^[a-c]) ","",.x))) %>%
  filter(metric %in% c("pearson.correlation","MSSS","crpss")) %>% 
  mutate(metric.lbl=case_when(metric=="pearson.correlation" ~ "a) Pearson correlation",
                              metric=="MSSS"~ "b) MSESS",
                              metric=="crpss"~"c) CRPSS")) %>%
  #Ratchet lower value
  mutate(sig.lev=oob_squish(1-p.value,range=c(0.01,0.99)))

panel.lbls <- 
  uninit.comp %>% 
  distinct(metric.lbl) %>%
  mutate(panel.lbl=str_remove(metric.lbl," .*$")) %>% 
  mutate(x=-Inf,y=1)
  

#Make plot
  ggplot(uninit.comp,
       aes(x=lead,y=sig.lev,colour=species,shape=species,linetype=species))+
  facet_wrap(~ metric.lbl ,
             labeller = function(x) {x$metric.lbl <- str_remove(x$metric.lbl,"^.*\\) "); x})+
  geom_text(data=panel.lbls,
            hjust=-0.5,vjust=2,
            mapping=aes(x,y,label=panel.lbl),
            inherit.aes = FALSE)+
  labs(x="Lead time (years)",
       y="Significance level (Decadal skill > Uninit. skill)",
       colour="Species",shape="Species",linetype="Species")+
  theme(panel.spacing.x = unit(4,"mm"))+
  coord_cartesian(ylim=c(0.00999,0.99),xlim=c(0,10),expand=FALSE,clip="off")+
  # scale_y_log10(breaks=c(0.01,0.1,0.05,0.5,1),
  #               labels=function(.x) {
  #                 lbls <- sprintf("%2.0f%%",100*(1-.x))
  #                 lbls[1] <- paste(">",lbls[1])
  #                 return(lbls)},
  #               minor_breaks=rev(c(seq(0.01,0.1,by=0.01),seq(0.2,1,by=0.1))))+
  scale_y_continuous(trans=probit_trans(),
                     breaks=c(0.01,0.05,seq(0.1,0.9,by=0.2),0.95,0.99),
                     minor_breaks=c(seq(0.01,0.1,by=0.01),
                                    seq(0.1,0.9,by=0.1),
                                    seq(0.9,0.99,by=0.01)),
                     labels=function(.x) {
                       lbls <- sprintf("%2.0f%%",100*.x)
                       lbls[1] <- paste("<",lbls[1])
                       lbls[length(lbls)] <- paste(">",lbls[length(lbls)])
                       return(lbls)})+
    scale_x_continuous(breaks=seq(0,10,by=2))+
  theme(panel.border = element_blank())+
  geom_rect(xmin=-Inf,xmax=Inf,ymin=-Inf,ymax=Inf,fill=NA,
            colour="grey20",size=0.25,show.legend = FALSE)+
  geom_line()+
  geom_point()
  ...
  uninit.comp %>% 
    select(metric=metric.lbl, species,lead,value=sig.lev) %>% 
    write_csv2(file=here("manuscript","source_data","Ext.Fig.3.csv"))
  
```

# Table of uninitialized models
```{r}
#Load table of source IDs to get institution IDs
#(this should be quicker than trying to get it directly from the metadata)
#URL: https://raw.githubusercontent.com/WCRP-CMIP/CMIP6_CVs/master/CMIP6_source_id.json
#See also  https://pcmdi.llnl.gov/CMIP6/TermsOfUse/TermsOfUse6-1.html
CMIP6.tbl <- 
  jsonlite::fromJSON(here("manuscript/CMIP6_source_id.json"),
           flatten=TRUE) %>% 
  extract2("source_id") %>% 
  enframe(name="id") %>% 
  unnest_wider(value) %>% 
  select(source_id,institution_id) %>% 
  mutate(institution_id=map_chr(institution_id,~paste(.x,collapse=", ")))

#Merge in with model list
CMIP6.mdls <- 
  these.cfg %>% 
  filter(fig=="skill") %>%
  mutate(path=map(pcfg, ~ .x@data.sources[["CMIP6.uninit"]]@sources)) %>% 
  select(lbl,var,path) %>% 
  unnest(path) %>%
  mutate(fnames=basename(path)) %>% 
  separate(fnames,c("variable","table","source_id","experiment",
                    "variant_label","grid_label","dates"),sep="_") %>% 
  distinct(variable,source_id,variant_label,grid_label) %>% 
  left_join(y=CMIP6.tbl)
...
CMIP6.mdls %>% 
  mutate(exists="\U2713") %>% 
  pivot_wider(names_from=variable,values_from = exists,values_fill = list(exists="")) %>%
  arrange(source_id) %>%
...
this.p.smooth <- 
  ...
  guides(shape=guide_legend(title="Mean window"),
         ...
         linetype=guide_legend(title="Mean window",
                               title.position = "top")) +
    labs(y="",title="Pearson correlation coefficient (r)")
#this.p.smooth$layers[[1]]$data$panel.lbl <- sprintf("%s)",letters[c(2,4,6)])

#Modify fixed.lead plot
this.fixed.lead <-
 fixed.lead.plt +
    facet_wrap(~lbl,ncol=1,
               strip.position="right",
  ...
  labs(y="",title="Habitat area (10³ km²)")+
  scale_x_continuous(expand=c(0,0),limits=c(1960,2020))+
  scale_y_continuous(expand=c(0,0),
                     labels=function(x) {x/1000},
                     limits=function(x) {range(pretty(x))})
this.fixed.lead$layers[[5]]$data$panel.lbl <- sprintf("%s)",letters[4:6])


#Patchwork 
wrap_plots(this.p.smooth,this.fixed.lead,ncol=2) +
