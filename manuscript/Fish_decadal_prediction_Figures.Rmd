...
for(this.cfg in plt.cfgs) {
  ...
  signif.lbls <- c("p < 0.005","p < 0.01", "p < 0.05", "ns")
  ...
  g <- 
    ...
    annotate("rect",xmin=0,xmax=10,ymin=1,ymax=max(this.cfg$ylim),fill="white",colour="black")+
    ...
    geom_point(data=signif.dat,
               mapping=aes(x=lead.yrs,y=mean(c(1,max(this.cfg$ylim))),shape=p.txt,size=p.txt))+
...
smooth.dat <-
  ...
  mutate(smoothing=case_when(is.na(smoothing) ~ "1 year",
         ...
         ylbl=case_when(metric=="pearson.correlation"~ "Pearson correlation (r)",
                        metric=="MSSS"~ "MSESS"),
         ylbl=fct_relevel(ylbl,"Pearson correlation (r)"),
         lbl=factor(lbl),
         lbl=fct_relabel(lbl,function(x) gsub("^[a-c]) ","",x)))
  
  facet.lbls <-
    expand_grid(ylbl=levels(smooth.dat$ylbl),
                lbl=levels(smooth.dat$lbl)) %>%
    mutate(this.lbl=sprintf("%s)",letters[1:nrow(.)]),
           x=0.1,y=0.99,
           ylbl=factor(ylbl,levels(smooth.dat$ylbl)),
           lbl=factor(lbl,levels(smooth.dat$lbl)))
  ...
  g <- 
    ...
    labs(x="Lead time (years)",
         y="")+
    #Lines
    geom_line(data=smooth.dat,
              mapping=aes(x=lead.yr,
                          y=value,
                          colour=dispName,
                          linetype=smoothing))+
    scale_linetype_manual(values=c(1,2,3))+
    guides(colour=guide_legend(title="Metrics",
                               title.position = "left",
                               order=1),
           linetype=guide_legend(title="Rolling mean",
                                 title.position = "left")) 
...
forecast.cfg.l$Bluefin <- 
  tibble(yr.rng=list(1980:2010),
         starts=list(c(1982,1992,2002)),
         scalar=-1,
         lbl2="Bluefin Habitat Extent (deg Lat W)")


# forecast.cfg.l$Blue_whiting_decadal <- 
#   tibble(yr.rng=list(1980:2020),
#          starts=list(c(2004,2012)),
#          scalar=1,
#          lbl2="Blue whiting spawning habitat (km²)")

forecast.cfg.l$Mackerel_summer <- 
  tibble(yr.rng=list(1980:2010),
         starts=list(c(1990,2000,2010)),
         scalar=1/1e6,
         lbl2="Mackerel habitat (Mkm²)")
...
for(this.cfg in forecast.cfg) {
  ...
  this.obs <- 
    ...
    select(year,Value=value,"Rolling mean"=rollmean) %>%
    pivot_longer(-year) %>%
  ...
  forecast.plt[[this.cfg$name]] <- 
    ...
    geom_line(data=forecast.realmean,
              mapping=aes(year,value,col=startYear),show.legend = FALSE)  +
    new_scale_color()+
    #Observations
    ...
    guides(size=guide_legend(name="Observations",order=1),
           shape=guide_legend(name="Observations",order=1),
           linetype=guide_legend(name="Observations",order=1),
           colour=guide_legend(name="Observations",order=1))
...
for(plt.cfg in sp.plt.cfg) {
  ...
  #sp.plots[[length(sp.plots)+1]] <-
    ...
    #              ggplot2:::binned_pal(scales::manual_pal(c("white",plt.cfg$fill.pal))),
    #              guide="coloursteps",
